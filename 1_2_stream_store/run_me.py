...
class spotifyProject(object):
    ...
    def get_rel_playlist_owner(self):
        return map(partial(get_in, ['owner', 'id']),
                   get_in(['playlists', 'items'], loads(self.spotify.category_new_releases), []))

    def get_rel_playlist_id(self):
        return map(partial(get_in, ['id']),
                 get_in(['playlists', 'items'], loads(self.spotify.category_new_releases), []))

    def get_user_playlist_owner(self):
        return map(partial(get_in, ['owner', 'id']),
                   get_in(['items'], loads(self.spotify.user_playlists), []))
...
def do_it_all(user_fetch=False):
    sp = spotifyProject()
    sp.loadConstants()
    sp.loadYaml()
    sp.setAwsUp()
